export default {
  translation: {
    ...
    "Show others this was sent via Imwald": "Показать другим, что сообщение отправлено через Imwald",
    ...
    "read relays description": "Ретрансляторы для чтения используются для получения событий о вас. Другие пользователи публикуют события, которые вы должны увидеть, через ваши ретрансляторы для чтения.",
    "write relays description": "Ретрансляторы для записи используются для публикации ваших событий. Другие пользователи получают ваши события через ретрансляторы для записи.",
    "read & write relays notice": "Количество серверов для чтения и записи должно быть оптимально от 2 до 4.",
    "Tor & I2P": "Tor & I2P",
    ...
    "Enter the password to decrypt your ncryptsec": "Введите пароль для расшифровки вашего ncryptsec",
    Back: "Назад",
    ...
    "Show this category expanded on payment method lists; other categories collapse behind an accordion.": "Показывать эту категорию развёрнутой в списках способов оплаты; остальные сворачиваются в аккордеон.",
    ...
    "Are you sure you want to unfollow this user?": "Вы уверены, что хотите отписаться от этого пользователя?",
    ...
    "Successfully republish to your write relays": "Успешно ретранслировано в ваши ретрансляторы для записи",
    "Failed to republish to your write relays: {{error}}": "Не удалось ретранслировать в ваши ретрансляторы для записи: {{error}}",
    "Successfully republish to relay set: {{name}}": "Успешно ретранслировано в набор ретрансляторов: {{name}}",
    "Failed to republish to relay set: {{name}}. Error: {{error}}": "Не удалось ретранслировать в набор ретрансляторов: {{name}}. Ошибка: {{error}}",
    "Successfully republish to relay: {{url}}": "Успешно ретранслировано в ретранслятор: {{url}}",
    "Failed to republish to relay: {{url}}. Error: {{error}}": "Не удалось ретранслировать в ретранслятор: {{url}}. Ошибка: {{error}}",
    "Write relays": "Ретрансляторы для записи",
    "n more boosts": "{{count}} more boosts",
    FollowListNotFoundConfirmation: "Список подписок не найден. Хотите создать новый? Если вы уже подписывались на пользователей ранее, пожалуйста, НЕ подтверждайте, так как эта операция приведет к потере вашего предыдущего списка подписок.",
    MuteListNotFoundConfirmation: "Список заблокированных не найден. Хотите создать новый? Если вы уже блокировали пользователей ранее, пожалуйста, НЕ подтверждайте, так как эта операция приведет к потере вашего предыдущего списка заблокированных.",
    ...
    "Your current relay configuration may not be optimal. This could make it difficult for others to find your posts and may result in incomplete notifications.": "Ваша текущая конфигурация релеев может быть не оптимальной. Это может затруднить поиск ваших сообщений другими пользователями и привести к неполным уведомлениям.",
    ...
    "This note mentions a user you muted": "Эта заметка упоминает пользователя, которого вы заглушили",
    Filter: "Фильтр",
    ...
    "No reviews yet. Be the first to write one!": "Отзывов пока нет. Станьте первым, кто напишет отзыв!",
