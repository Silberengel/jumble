export default {
  translation: {
    ...
    "Relay pulse active npubs hint": "Kind 0 profiles for pubkeys seen on your favorite relays in the last hour (same sample as Relay pulse).",
    ...
    "Relay auth accepted (NIP-42)": "The relay accepted authentication (NIP-42): {{relay}}{{detailSuffix}}",
    ...
    "Create a calendar event and send an invite. The recipient will see the event with a join link.": "Create a calendar event and send an invite. The recipient will see the event with a join link.",
    "Schedule a video call": "Schedule a video call",
    "Create a calendar event and send kind 24 invites to each listed invitee.": "Create a calendar event and send kind 24 invites to each listed invitee.",
    Invitees: "Invitees",
    "Paste nostr:npub1... or nostr:nprofile1... (one or more)": "Paste nostr:npub1... or nostr:nprofile1... (one or more)",
    ...
    "Create a calendar event and send an invite. No video link — for real-life meetups, conferences, etc.": "Create a calendar event and send an invite. No video link — for real-life meetups, conferences, etc.",
    ...
    "Create a calendar event for a real-life meetup and send kind 24 invites to each invitee.": "Create a calendar event for a real-life meetup and send kind 24 invites to each invitee.",
    ...
    "Reply to": "Ответить",
    Search: "Поиск",
    "The relays you are connected to do not support search": "Подключённые ретрансляторы не поддерживают поиск",
    ...
    "A special note for picture-first clients like Olas": "Особенная заметка для клиентов с приоритетом изображений, таких как Olas",
    ...
    "This is a private key. Do not share it with anyone. Keep it safe and secure. You will not be able to recover it if you lose it.": "Это приватный ключ. Не делитесь им с другими. Храните его в надёжном месте – восстановить его будет невозможно.",
    Edit: "Редактировать",
    Save: "Сохранить",
    ...
    "If enabled, you can zap with a single click. Click and hold for custom amounts": "Если включено, вы можете отправить зап одним нажатием. Нажмите и удерживайте для выбора суммы",
    All: "Все",
    Reactions: "Реакции",
    Zaps: "Запы",
    Boosts: "Бусты",
    ...
    "Your donation helps me maintain Imwald and make it better! 😊": "Ваше пожертвование помогает поддерживать и улучшать Imwald! 😊",
    ...
    "External relay search is not available for this link type": "External relay search is not available for this link type.",
    "Searched external relays not found": "Searched {{count}} external relays; the note was still not found.",
    ...
    "You are about to publish an event signed by [{{eventAuthorName}}]. You are currently logged in as [{{currentUsername}}]. Are you sure?": "Вы собираетесь опубликовать событие, подписанное [{{eventAuthorName}}]. Вы в данный момент вошли как [{{currentUsername}}]. Вы уверены?",
    ...
    "Only show content from your followed users and the users they follow": "Показывать только контент от пользователей, на которых вы подписаны, и от пользователей, на которых они подписаны",
    ...
    Balance: "Баланс",
    characters: "символов",
    translateApiKeyDescription: "Вы можете использовать этот API-ключ в любом другом месте, которое поддерживает LibreTranslate. URL сервиса: {{serviceUrl}}",
    "Top up": "Пополнить",
    ...
    "Are you sure you want to reset your API key? This action cannot be undone.": "Вы уверены, что хотите сбросить ваш API-ключ? Это действие не может быть отменено.",
    Warning: "Предупреждение",
    "Errors & warnings": "Errors & warnings",
    "Your current API key will become invalid immediately, and any applications using it will stop working until you update them with the new key.": "Ваш текущий API-ключ станет недействительным немедленно, и любые приложения, использующие его, перестанут работать, пока вы не обновите их новым ключом.",
    "Service address": "Адрес сервиса",
    Pay: "Оплатить",
    interactions: "взаимодействия",
    notifications: "уведомления",
    notificationsViewAsAccount: "View as",
    notificationsViewAsAccountAria: "Switch stored account. Notifications, replies, zaps, reactions, and boosts use this account.",
    ...
    "Currently hiding {type} from untrusted users.": "В настоящее время скрываются {{type}} от недоверенных пользователей.",
    ...
    "Click continue to hide {type} from untrusted users.": "Нажмите \"Продолжить\", чтобы скрыть {{type}} от недоверенных пользователей.",
    "Trusted users include people you follow and people they follow.": "Доверенные пользователи включают людей, на которых вы подписаны, и людей, на которых они подписаны.",
    Continue: "Продолжить",
    ...
    "You need to add at least one blossom server in order to upload media files.": "Вам нужно добавить хотя бы один сервер Blossom, чтобы загружать медиафайлы.",
    ...
    "Unlike regular notes, polls are not widely supported and may not display on other clients.": "В отличие от обычных заметок, опросы не получили широкой поддержки и могут не отображаться в других клиентах.",
    ...
    "After changing emojis, you may need to refresh the page": "После изменения эмодзи может потребоваться обновить страницу",
    "Too many read relays": "Слишком много релеев для чтения",
    "Too many write relays": "Слишком много релеев для записи",
    "You have {{count}} read relays. Most clients only use 2-4 relays, setting more is unnecessary.": "У вас {{count}} релеев для чтения. Большинство клиентов используют только 2-4 релея, устанавливать больше не нужно.",
    "You have {{count}} write relays. Most clients only use 2-4 relays, setting more is unnecessary.": "У вас {{count}} релеев для записи. Большинство клиентов используют только 2-4 релея, устанавливать больше не нужно.",
    ...
    "Release notes use the editor below (optional).": "Release notes use the editor below (optional).",
    ...
    "See extra info for each notification": "Просмотреть дополнительную информацию для каждого уведомления",
    ...
    "Write relays and {{count}} other relays": "Ретрансляторы записи и {{count}} других ретрансляторов",
    ...
    "Home trending slice notice": "This feed includes a trending slice (nostrarchives). Notes that reached you from that relay show “Trending on Nostr” under the card.",
    Sort: "Sort",
    newest: "newest",
    oldest: "oldest",
    ...
    "Set up your wallet to send and receive sats!": "Настройте свой кошелёк, чтобы отправлять и получать саты!",
    "Set up": "Настроить",
    ...
    "COUNT spell relay errors hint": "One or more relays failed or returned an error. You can change the relay list in the spell and save again.",
    "COUNT spell total distinct explanation": "Distinct matching event IDs across all relays that responded successfully (duplicates across relays removed). Each relay only returns up to the filter limit.",
    "Leave empty to use your write relays.": "Leave empty to use your write relays.",
    "COUNT spell result explanation": "Distinct events returned for this filter (merged from your spell relays and default write relays, duplicates removed). Relays only return up to the filter limit.",
    "COUNT spell may be capped by limit": "This count may equal your spell limit — there could be more matching events on the network.",
    "Spell count failed. Check relays or try again.": "Could not complete the count. Check relays or try again.",
    ...
    "Article URLs Nostr manual subtitle": "Article URLs with no subscribed-feed items yet — from Nostr relay discovery or links you added. Anything that already has feed items appears under RSS.",
    "RSS feed column title": "RSS & linked articles",
    "RSS feed column subtitle": "Feed entries and article cards that include at least one item from your subscribed RSS feeds.",
