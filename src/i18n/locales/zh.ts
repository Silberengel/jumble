export default {
  translation: {
    ...
    Replies: "回复",
    "Using private key login is insecure. It is recommended to use a browser extension for login, such as alby, nostr-keyx or nos2x. If you must use a private key, please set a password for encryption at minimum.": "使用私钥登录不安全。建议使用浏览器插件登录，如 alby、nostr-keyx 或 nos2x。如果必须使用私钥，请至少设置一个密码进行加密。",
    ...
    "read relays description": "读服务器用于寻找与您有关的事件。其他用户会将想要你看到的事件发布到您的读服务器。",
    "write relays description": "写服务器用于发布您的事件。其他用户会从您的写服务器寻找您发布的事件。",
    ...
    "Show this category expanded on payment method lists; other categories collapse behind an accordion.": "在支付方式列表中展开此类别；其他类别折叠在手风琴中。",
    ...
    "Failed to republish to relay set: {{name}}. Error: {{error}}": "重新发布到服务器组: {{name}} 失败。错误: {{error}}",
    "Successfully republish to relay: {{url}}": "成功重新发布到服务器: {{url}}",
    "Failed to republish to relay: {{url}}. Error: {{error}}": "重新发布到服务器: {{url}} 失败。错误: {{error}}",
    "Write relays": "写服务器",
    "n more boosts": "{{count}} more boosts",
    FollowListNotFoundConfirmation: "未找到关注列表。你想创建一个新的吗？如果你之前已经关注了用户，请不要确认，因为此操作会导致你丢失之前的关注列表。",
    MuteListNotFoundConfirmation: "未找到屏蔽列表。你想创建一个新的吗？如果你之前已经屏蔽了用户，请不要确认，因为此操作会导致你丢失之前的屏蔽列表。",
    ...
    "Your current relay configuration may not be optimal. This could make it difficult for others to find your posts and may result in incomplete notifications.": "您当前的中继配置可能不是最优的。这可能导致其他人难以找到您发布的内容，也可能导致您无法获取完整的通知列表。",
    "Optimize Now": "立即优化",
    "Maybe Later": "稍后处理",
    ...
    "Personal Lists": "个人列表",
    "Personal lists hub intro": "静音列表、关注的人、NIP-51 书签、置顶、兴趣主题（kind 10015）、NIP-30 用户表情列表（kind 10030）与表情包（kind 30030）。网页书签（NIP-B0，kind 39701）另计：可在文章侧栏保存，或在「咒语」里打开书签流同时查看笔记书签与网页书签。",
    "Mute list": "静音列表",
    "Following list": "关注列表",
    ...
    "User emoji list intro": "NIP-30：内联自定义表情（`emoji` 标签）与指向 kind 30030 包的引用（`a` 标签）。编辑完成后点击发布。",
    ...
    Shortcode: "短码",
    "Emoji sets settings intro": "NIP-30 表情包（kind 30030）：每个集合有 `d` 标签与若干 `emoji`（短码 + 图片 URL）。在编辑对话框中保存并发布。",
    "New emoji set": "新建表情包",
